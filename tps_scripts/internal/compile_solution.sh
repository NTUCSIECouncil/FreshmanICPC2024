...
if [ "${LANG}" == "cpp" ] ; then
	...
	if "${HAS_GRADER}"; then
		...
		vrun capture_compile ${CXX} ${CPP_OPTS} -c "${grader_cpp}" -o "${GRADER_NAME}.o" "${coloring_flag}"
	...
	vrun capture_compile ${CXX} ${CPP_OPTS} "${files_to_compile[@]}" -o "${exe_file}" "${coloring_flag}"
